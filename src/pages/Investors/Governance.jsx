...
const Governance = () => {
  ...
  const renderBoard = () => (
    ...
    >
      {[
        ...
        { name: "Rajneesh Gupta", role: "Chief Financial Officer", img: rajneeshImg },
        { name: "Sunil Suri", role: "Independent Director", img: sunilImg },
        { name: "Hetvi Ketan Sheth", role: "Independent Director", img: hetviImg },
        ...
        >
          ...
          <div>
            ...
            <p className="text-[14px] text-gray-500 mt-1 font-medium">{member.role}</p>
          </div>
        </motion.div>
      ))}
    </motion.div>
    ...
    <div className="grid grid-cols-1 gap-8">
      {[
        { 
          ...
          members: [
            {n: "Hetvi Ketan Sheth", p: "Chairman", d: "Independent Director"},
            {n: "Sunil Suri", p: "Member", d: "Independent Director"},
            {n: "Binny Gupta", p: "Member", d: "Chairman and Managing Director"}
          ] 
        },
        { 
        ...
        >
          <h3 className="text-[16px] font-bold mb-6 text-[#292C44] border-b border-gray-200 pb-3 tracking-wide uppercase">{comm.title}</h3>
              ...
              <tbody className="text-gray-700">
                {comm.members.map((m, idx) => (
                  <tr key={idx} className="border-t border-gray-200/60">
                    ...
                    <td className={`py-4 pr-4 font-medium ${m.p === "Chairman" ? "text-blue-600" : ""}`}>{m.p}</td>
  ...
  const renderContact = () => (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="p-10 bg-[#292C44] text-white rounded-2xl shadow-xl relative overflow-hidden">
      <div className="relative z-10">
        <h3 className="text-2xl font-bold mb-1">Sakshi Srivastava</h3>
        <p className="text-[15px] opacity-80 mb-6 font-medium">Company Secretary and Compliance Officer</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8 pt-8 border-t border-white/10">
          <div>
            ...
            <p className="text-[16px] font-semibold">compliance@rennystrips.com</p>
            <p className="text-[16px] font-semibold mt-1">+91 6283368523</p>
          </div>
          <div>
            <h5 className="text-[12px] uppercase tracking-widest opacity-60 mb-2">Registered & Corporate Office</h5>
            <p className="text-[14px] leading-relaxed opacity-90">
              Khasra No 34//6/2 (0-8)-7 (8-0)-9 and Khata No 121/127,<br/>
              Lakhowal Road, Kohara,<br/>
              Ludhiana-141112
            </p>
          </div>
        </div>
              ...
              {GOVERNANCE_TABS.map((tab) => (
                <button
                  ...
                  className={`px-6 py-3 rounded-t-lg text-[14px] font-bold transition-all relative ${
                    activeTab === tab.key ? "text-[#292C44]" : "text-gray-400 hover:text-gray-600"
                  }`}
                >
                  {tab.label}
                  {activeTab === tab.key && (
