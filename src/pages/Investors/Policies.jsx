    ...
    <div className="font-['Helvetica','Arial',sans-serif]">
      ...
      <section className="relative w-full h-[40vh] lg:h-[100vh] overflow-hidden">
        ...
        <div className="absolute bottom-16 left-0 w-full px-6 lg:px-20 z-10">
          <motion.h1 initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className={`text-white ${mainHeadingFont}`}>
            Our Policies
          </motion.h1>
        </div>
      </section>
              ...
              <div ref={scrollRef} className="flex overflow-x-auto gap-3 pb-2 no-scrollbar scroll-smooth w-full px-2">
                {FIXED_POLICIES.map((item) => (
                  <button
                    ...
                    className={`whitespace-nowrap px-6 py-3 rounded-lg text-[14px] font-bold transition-all border ${activeTab === item.slug ? "text-white shadow-lg border-transparent" : "bg-gray-50 text-gray-400 border-gray-100 hover:bg-gray-100"
                      }`}
              ...
              <motion.div key={activeTab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
                {activeDocs.length > 0 ? (
                  activeDocs.map((policy) => (
                    <div key={policy._id} className="p-6 md:p-10 bg-gray-50 rounded-2xl border border-gray-100 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-6 md:gap-8">
                      <div className="flex-1">
                        <h3 className={`${subHeadingFont} text-gray-900 mb-2 leading-snug`}>{policy.docName}</h3>
                        <p className="text-[13px] text-gray-400 font-medium italic">Category: {policy.label}</p>
                      </div>
                      <a href={policy.url} target="_blank" rel="noreferrer" className="shrink-0">
