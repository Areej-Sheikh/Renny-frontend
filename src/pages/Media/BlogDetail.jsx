        ...
        <main className="w-full lg:w-3/5">
          ...
          <div className="space-y-3">
            {blog.bodySections?.map((section, index) => {
              const prevType = index > 0 ? blog.bodySections[index - 1].type : null;
              const isConsecutiveParagraphs = section.type === "paragraph" && prevType === "paragraph";
              const isConsecutiveSubPoints = section.type === "sub-point" && prevType === "sub-point";
              const isParagraphToSubPoint = section.type === "sub-point" && prevType === "paragraph";
              const isSubPointToParagraph = section.type === "paragraph" && prevType === "sub-point";
                                ...
                                {item.descriptions && item.descriptions.length > 0 ? (
                                  <div className="space-y-2 mt-2">
                                    {item.descriptions.map((desc, dIdx) => (
                                      <p key={dIdx} className="leading-relaxed text-gray-700 text-sm md:text-base">
                                        {desc}
                                      </p>
                                    ))}
                                  </div>
                ...
                {section.type === "table" && (
                  <div className="overflow-x-auto my-6">
                    {typeof section.table === "string" ? (
                      <div dangerouslySetInnerHTML={{ __html: section.table }} className="table-auto w-full text-left prose prose-table:border-collapse prose-td:border prose-th:border prose-td:p-2 prose-th:p-2" />
                          ...
                          {section.table.map((row, rIdx) => (
                            <tr key={rIdx} className={rIdx === 0 ? "bg-gray-100 font-bold" : ""}>
                              {Array.isArray(row) ? row.map((cell, cIdx) => (
                                ...
                                <td className="border border-gray-300 px-4 py-2">{JSON.stringify(row)}</td>
                              )}
                            </tr>
                          ))}
                        </tbody>
                      </table>
                    ) : section.table && typeof section.table === 'object' && section.table.rows ? (
                      <table className="min-w-full border-collapse border border-gray-300">
                        {section.table.headers && (
                           <thead className="bg-gray-100">
                             <tr>
                               {section.table.headers.map((h, i) => (
                                 <th key={i} className="border border-gray-300 px-4 py-2 text-left">{h}</th>
                               ))}
                             </tr>
                           </thead>
                        )}
                      ...
                      <div dangerouslySetInnerHTML={{ __html: section.content }} className="table-auto w-full text-left prose prose-table:border-collapse prose-td:border prose-th:border prose-td:p-2 prose-th:p-2" />
                    ) : (
                      <div className="p-4 border border-dashed border-gray-300 text-gray-500 text-center">Table data is empty or invalid.</div>
                    )}
                  </div>
                )}
              </div>
            ...
            <div className="flex flex-col gap-2">
              {relatedBlogs.map((rel, idx) => (
                ...
                >
                  <button
                    onClick={() =>
                      setOpenRelatedIndex(openRelatedIndex === idx ? null : idx)
                    }
